...
export default function SignUp() {
    ...
    const handleSubmit=(event)=>
    {
      ...
      if(name.length===0 ||  phno.length!==10  || emailid.length===0 ||  password.length===0 || password!==confirmpassword)
      {
        seterror(true);
      }
      else
  ...
  return (
    ...
    NAME :<br/> <input type="text" value={name} placeholder="FIRST NAME" onChange={e=>setName(e.target.value)}/><br/>
    <div>
    {error && name.length===0?
      <label style={{color:'red'}}>Name is required</label>:""} 
    </div><br/>
    MOBILE NUMBER:<br/><input type="number" value={phno} placeholder="MOBILE NUMBER" onChange={e=>setMobileNum(e.target.value)}/><br/>
    <div>
...
{error && !gender ? 
  <label style={{ color: 'red' }}>Gender is required</label>:""}
    ...
    E-MAIL ID:<br/><input type="email" value={emailid} placeholder="E-MAIL ID" onChange={e=>setEmail(e.target.value)}/><br/>
    <div>
